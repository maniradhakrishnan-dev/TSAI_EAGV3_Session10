    ...
    def chat(self, prompt: str = None, *,
        ...
        for attempt in range(8):
            try:
                ...
                if r.status_code in (429, 502, 503, 504) and attempt < 7:
                    wait_time = 1.5 + attempt * 2.0 + random.random()
                    print(f"[LLM.chat] Gateway returned {r.status_code}. Retrying in {wait_time:.1f}s...")
                ...
                if status_code in (429, 502, 503, 504) and attempt < 7:
                    wait_time = 1.5 + attempt * 2.0 + random.random()
                    print(f"[LLM.chat] Gateway error: {e}. Retrying in {wait_time:.1f}s...")
                    time.sleep(wait_time)
                    continue
                raise
    ...
    def embed(self, text: str,
        ...
        for attempt in range(8):
            try:
                ...
                if r.status_code in (429, 502, 503, 504) and attempt < 7:
                    wait_time = 1.5 + attempt * 2.0 + random.random()
                    print(f"[LLM.embed] Gateway returned {r.status_code}. Retrying in {wait_time:.1f}s...")
                    time.sleep(wait_time)
                    continue
                r.raise_for_status()
                return r.json()
                ...
                if status_code in (429, 502, 503, 504) and attempt < 7:
                    wait_time = 1.5 + attempt * 2.0 + random.random()
                    print(f"[LLM.embed] Gateway error: {e}. Retrying in {wait_time:.1f}s...")
                    time.sleep(wait_time)
                    continue
                raise
