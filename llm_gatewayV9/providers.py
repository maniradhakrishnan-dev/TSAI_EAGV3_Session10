...
class OpenAICompatProvider(BaseProvider):
    ...
    async def chat(self, messages, *, max_tokens=2048, temperature=0.7, model=None,
        ...
        body = {
            ...
            "temperature": max(temperature, 0.01) if self.name == "nvidia" and temperature == 0.0 else temperature,
            "stream": False,
        }
        if tools:
        ...
        async with httpx.AsyncClient(timeout=45) as c:
            ...
            if r.status_code != 200:
                ...
                if r.status_code != 200 and "json_schema" in (body.get("response_format") or {}).get("type", ""):
                    body["response_format"] = {"type": "json_object"}
                    r = await c.post(f"{self.base_url}/chat/completions", headers=self._headers(), json=body)
                if r.status_code != 200 and "response_format" in body:
    ...
    def _translate_messages(self, messages, system_text):
        ...
        for m in messages:
            if m.get("role") == "system":
                inline_system.append(m.get("content", ""))
        if inline_system:
            if system_content:
                system_content += "\n\n" + "\n\n".join(inline_system)
            else:
        ...
        for m in messages:
            ...
            if r == "user":
                content = m.get("content", "")
                if system_content and not system_prepended:
                    if isinstance(content, list):
        ...
        if system_content and not system_prepended:
            out.insert(0, {"role": "user", "content": system_content})
        return out
...
def _gemini_thinking_knob(model: str) -> Optional[str]:
    ...
    if "flash-lite" in m:
        return None
    if "2.5-pro" in m or "3-pro" in m or "3.1-pro" in m or "3.5-pro" in m:
        return "level"
    if "3-flash" in m or "3.1-flash" in m or "3.5-flash" in m:
