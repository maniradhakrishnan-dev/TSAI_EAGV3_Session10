...
async def chat(req: ChatRequest):
    ...
    if agent_pinned_provider and not req.provider:
        if agent_pinned_provider in candidates:
            candidates = [agent_pinned_provider] + [c for c in candidates if c != agent_pinned_provider]
            import asyncio as _asyncio
            deadline = time.time() + 30
            while time.time() < deadline:
                name, _ = router.pick(est, [agent_pinned_provider], required_caps=required_caps)
                if name is not None:
                    break
                snap = router.state[agent_pinned_provider].snapshot(LIMITS[agent_pinned_provider])
                cd = max(snap["cooldown_remaining"], snap["backoff_remaining"])
                if cd <= 0 or cd > 30:
                    break
    ...
    if explicit_override and len(candidates) == 1:
        ...
        while time.time() < deadline:
            ...
            if name is not None:
                break
            snap = router.state[candidates[0]].snapshot(LIMITS[candidates[0]])
            cd = max(snap["cooldown_remaining"], snap["backoff_remaining"])
            if cd <= 0 or cd > 65:
                break
            await _asyncio.sleep(min(cd + 0.05, 5))
    ...
    msg = f"all providers unavailable. attempts: {all_attempts}. last_error: {last_err}"
    print(f"\n[GATEWAY ERROR 503] {msg}\n", flush=True)
