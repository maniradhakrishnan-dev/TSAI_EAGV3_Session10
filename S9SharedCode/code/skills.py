...
def parse_skill_json(text: str) -> dict:
    ...
    if the model added them despite being told not to. Also handles cases where
    the model outputs conversational text surrounding one or more JSON blocks."""
    t = (text or "").strip()
    try:
        return json.loads(t)
    ...
    for b in blocks:
        try:
            parsed_objects.append(json.loads(b.strip()))
        except json.JSONDecodeError:
            pass
            
    if parsed_objects:
        for obj in parsed_objects:
            if isinstance(obj, dict) and ("nodes" in obj or "successors" in obj):
                return obj
        nodes = []
        for obj in parsed_objects:
            if isinstance(obj, dict) and "skill" in obj:
                nodes.append(obj)
        if nodes:
            return {"nodes": nodes, "rationale": "Extracted nodes from markdown blocks."}
        ...
        if char == "{":
            ...
            if stack:
                ...
                if not stack and start_idx != -1:
                    braces_blocks.append(text[start_idx:idx+1])
                    start_idx = -1
                    
    parsed_braces = []
    for b in braces_blocks:
        try:
            parsed_braces.append(json.loads(b))
        except json.JSONDecodeError:
            pass
            
    if parsed_braces:
        for obj in parsed_braces:
            if isinstance(obj, dict) and ("nodes" in obj or "successors" in obj):
                return obj
        nodes = []
        for obj in parsed_braces:
            if isinstance(obj, dict) and "skill" in obj:
                nodes.append(obj)
        if nodes:
            return {"nodes": nodes, "rationale": "Extracted nodes from scanned braces."}
            
        if len(parsed_braces) == 1 and isinstance(parsed_braces[0], dict):
            return parsed_braces[0]
...
async def run_skill(skill: Skill, node_id: str, graph_nodes,
    ...
    if skill.name == "browser":
        ...
        # Substitute upstream node values in metadata goal
        goal = node_spec.metadata.get("goal") or ""
        for r in resolved:
            if r.get("kind") == "upstream" and r.get("id"):
                inp_id = r["id"]
                val = r.get("output")
                if isinstance(val, dict):
                    output_str = val.get("content") or val.get("result") or json.dumps(val)
                else:
                    output_str = str(val)
                if " = " in output_str:
                    output_str = output_str.split(" = ")[-1].strip()
                goal = goal.replace(inp_id, output_str)
                label = graph_nodes[inp_id].get("metadata", {}).get("label")
                if label:
                    goal = goal.replace(f"n:{label}", output_str)
        node_spec.metadata["goal"] = goal
    ...
    if skill.name == "computer":
        ...
        for r in resolved:
            if r.get("kind") == "upstream" and r.get("id"):
                ...
                if " = " in output_str:
                    output_str = output_str.split(" = ")[-1].strip()
                # Try token substitution first
                old_goal = goal
                goal = goal.replace(inp_id, output_str)
                label = graph_nodes[inp_id].get("metadata", {}).get("label")
                if label:
                    goal = goal.replace(f"n:{label}", output_str)
                # If no substitution happened, track for appending
                if goal == old_goal:
                    upstream_values.append(output_str)
        # Append upstream results that weren't substituted via tokens
        if upstream_values:
            goal += f"\n\nIMPORTANT: The result from the previous step is: {', '.join(upstream_values)}. Type this exact value."
        node_spec.metadata["goal"] = goal
