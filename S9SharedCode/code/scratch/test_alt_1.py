...
def main():
    ...
    print(f"Activating Calculator (pid={pid}, wid={wid}) via EWMH...")
    ...
    # Press Alt + 1 using Xlib XTEST
    alt_keycode = d.keysym_to_keycode(0xffe9) # Alt_L
    one_keycode = d.keysym_to_keycode(0x0031) # 1
    
    print("Pressing Alt + 1...")
    xtest.fake_input(d, X.KeyPress, alt_keycode)
    xtest.fake_input(d, X.KeyPress, one_keycode)
    d.sync()
    time.sleep(0.1)
    
    xtest.fake_input(d, X.KeyRelease, one_keycode)
    ...
    with open("scratch/calc_ax_after_alt_1.txt", "w") as f:
        f.write(state.get("tree_markdown", ""))
    
    # Check lines count
    lines = state.get("tree_markdown", "").splitlines()
    print(f"AX tree saved to scratch/calc_ax_after_alt_1.txt. Total lines: {len(lines)}")
