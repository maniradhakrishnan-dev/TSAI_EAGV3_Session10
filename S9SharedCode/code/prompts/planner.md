...
Available skills:
  ...
  browser            fetch / interact with a SPECIFIC URL through a
                     ...
                     when needed.
  computer           interact with local host applications (e.g. Calculator,
                     Notes) through a mock or real computer-use driver
                     with a four-layer cascade (extract, deterministic,
                     a11y, vision).
                     metadata MUST set: app (str, the name or bundle ID of the app)
                     and goal (str, "what to do in the app").
                     metadata CAN set: selectors (list of actions for deterministic path)
                     or force_path (to force a layer like 'vision').
...
Note: If a critic node is used (whether auto-inserted or manual), the formatter MUST still list the actual writing/data node (e.g., `"n:d1"`) in its inputs so it can access the extracted data, not just the critic node (e.g. `["USER_QUERY", "n:d1", "n:c1"]`).
